...
async function validateNextJsConfig() {
  ...
  if (!configExists) {
    console.log(
      "⚠️  No next.config.js found. This might be okay, but standalone mode needs to be enabled."
    );
    console.log(
      '💡 Consider creating a next.config.js with output: "standalone"'
    );
  }
...
async function copyBackendAssets(resourcesDir) {
  ...
  if (!(await fs.pathExists(backendExe))) {
    console.warn(
      "⚠️  main.exe not found. This might cause issues in production."
    );
    console.log(
      "💡 Make sure your FastAPI app is compiled to main.exe in the dist folder"
    );
  }
...
async function verifyAndCreateManifest(resourcesDir) {
  const frontendServerJs = path.join(
    resourcesDir,
    "frontend/frontend",
    "server.js"
  );
  const frontendNextDir = path.join(resourcesDir, "frontend/frontend", ".next");
  ...
  const verification = {
    frontend: {
      ...
      packageJson: {
        exists: await fs.pathExists(
          path.join(resourcesDir, "frontend/frontend", "package.json")
        ),
        path: path.join(resourcesDir, "frontend/frontend", "package.json"),
      },
    },
    backend: {
      executable: {
  ...
  console.log(
    `   Frontend server.js: ${
      verification.frontend.serverJs.exists ? "✅" : "❌"
    }`
  );
  console.log(
    `   Frontend .next build: ${
      verification.frontend.nextBuild.exists ? "✅" : "❌"
    }`
  );
  console.log(
    `   Frontend package.json: ${
      verification.frontend.packageJson.exists ? "✅" : "❌"
    }`
  );
  console.log(
    `   Backend executable: ${
      verification.backend.executable.exists ? "✅" : "❌"
    }`
  );
  ...
  if (!verification.frontend.serverJs.exists) {
    ...
    console.log(
      '💡 This will cause a white screen. Ensure Next.js is configured with output: "standalone"'
    );
  }

  if (!verification.backend.executable.exists) {
    console.warn(
      "⚠️  Backend executable is missing. Backend functionality will not work."
    );
  }
}
