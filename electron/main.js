...
const logFile = path.join(isDev ? __dirname : path.dirname(app.getPath("exe")), "debug.log")
    ...
    try {
      if (isWindows) {
        ...
        exec(`taskkill /PID ${process.pid} /T /F`, (error) => {
          if (error) {
            log("warn", `Failed to taskkill process ${process.pid}: ${error.message}`)
          } else {
            log("info", `Successfully terminated process ${process.pid} with taskkill`)
          }
        ...
        setTimeout(() => {
          if (!process.killed && !resolved) {
            try {
              ...
              log("warn", `Failed to force kill process ${process.pid}: ${killError.message}`)
            }
          }
          setTimeout(resolveOnce, 1000)
        }, timeout / 2)
        ...
        exec(`kill -TERM ${pids.join(" ")}`, (termError) => {
          ...
          setTimeout(() => {
            exec(`kill -9 ${pids.join(" ")} 2>/dev/null`, (killError) => {
              if (killError) {
                log("debug", `SIGKILL completed (some processes may have already exited)`)
              } else {
...
async function cleanupProcesses() {
  ...
  try {
    ...
    await Promise.all([killProcessOnPort(FRONTEND_PORT), killProcessOnPort(BACKEND_PORT)])
    ...
    if (trackedProcesses.size > 0) {
      ...
      const trackedCleanup = Array.from(trackedProcesses.entries()).map(([pid, data]) => {
        return new Promise((resolve) => {
          try {
            if (data.cleanup) {
              data.cleanup()
            ...
            log("warn", `Error cleaning up tracked process ${pid}: ${error.message}`)
            resolve()
          }
        })
      })
...
function startFrontend() {
  return new Promise((resolve, reject) => {
    ...
    frontendProcess.stderr.on("data", (data) => {
      ...
      if (
        !resolved &&
        (output.includes("Ready in") || output.includes("started server") || output.includes("listening on"))
      ) {
